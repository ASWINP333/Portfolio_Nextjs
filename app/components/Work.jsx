    ...
    id="work" className="w-full px-[12%] py-10 scroll-mt-20">
      ...
      className="grid grid-cols-auto my-10 gap-5 dark:text-black">
        ...
        transition={{ duration: 0.3 }}
            ...
            relative cursor-pointer group border-[0.5px] border-gray-400"
            style={{ backgroundImage: `url(${project.bgImage})` }}
          >
            <div className='bg-blue-100 w-10/12 rounded-md absolute bottom-5
            left-1/2 border-s-black -translate-x-1/2 py-1 px-1.5 opacity-90 flex items-center justify-between
            duration-500 group-hover:bottom-7 hover:opacity-100
            dark:text-white dark:border-white dark:hover:bg-blue-200 '>
              <div>
                <h2 className='font-semibold text-black'>{project.title}</h2>
                {/* <p className='text-sm text-gray-700'>{project.description}</p> */}
                <button className='bg-slate-800 text-cyan-50 px-5
                rounded-full group-hover:bottom-7 hover:bg-slate-400
                dark:text-white dark:border-white dark:hover:bg-blue-300 '>
                <a href={project.description} target='#'>git</a></button>
              </div>
              ...
              transition'>
                <a href={project.live} target='#'><Image src={assets.send_icon} alt="send icon" className="w-5" /></a>
              </div>
            </div>
          </motion.div>
        ))}
